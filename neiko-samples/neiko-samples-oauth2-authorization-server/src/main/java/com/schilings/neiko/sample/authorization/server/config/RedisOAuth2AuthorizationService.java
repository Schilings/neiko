...
public class RedisOAuth2AuthorizationService implements OAuth2AuthorizationService {
	...
	public static final String AUTHORIZATIONS_PREFIX = RedisOAuth2AuthorizationService.class.getSimpleName()
			+ ".authorizations";

	public static final String INITIALIZED_AUTHORIZATIONS_PREFIX = RedisOAuth2AuthorizationService.class.getName()
			+ ".initializedAuthorizations";
	...
	private void saveAuthorization(OAuth2Authorization authorization) throws JsonProcessingException {
		getHashOperations().put(getAuthorizationsPrefix("all"), getAuthorizationsPrefix(authorization.getId()),
	...
	private void saveInitializedAuthorization(OAuth2Authorization authorization) throws JsonProcessingException {
		getHashOperations().put(getInitializedAuthorizationsPrefix("all"),
				getInitializedAuthorizationsPrefix(authorization.getId()),
				objectMapper.writeValueAsString(authorization));
	}
	...
	private void removeInitializedAuthorization(OAuth2Authorization authorization) {
		getHashOperations().delete(getInitializedAuthorizationsPrefix("all"),
				getInitializedAuthorizationsPrefix(authorization.getId()));
	}

	private OAuth2Authorization getAuthorization(String id) throws JsonProcessingException {
		return objectMapper.readValue(
				getHashOperations().get(getAuthorizationsPrefix("all"), getAuthorizationsPrefix(id)),
				OAuth2Authorization.class);
	}

	@SneakyThrows
	private OAuth2Authorization getInitializedAuthorization(String id) {
		return objectMapper.readValue(getHashOperations().get(getInitializedAuthorizationsPrefix("all"),
				getInitializedAuthorizationsPrefix(id)), OAuth2Authorization.class);
