...
public class WebClientConfiguration {
		...
		return WebClient.builder()
				...
				.filter((request, next) -> {
					ClientRequest filtered = ClientRequest.from(request)
							.header("Authorization", tokenHolder.getTokenWithPrefix()).build();
	...
	public HttpServiceProxyFactory httpServiceProxyFactory(WebClient webClient) {
		return HttpServiceProxyFactory.builder(WebClientAdapter.forClient(webClient)).embeddedValueResolver(str -> {
			// 动态分配
			...
			return str;
				...
				.customArgumentResolver(new RequestParameterObjectArgumentResolver()).build();
	}
