...
public interface ExtendMapper<T> extends BaseMapper<T> {
	...
	<DTO> List<DTO> selectJoinList(@Param(Constant.CLAZZ) Class<DTO> clazz,
			@Param(Constant.RESULT_MAP) String resultMap, @Param(Constants.WRAPPER) NeikoBaseJoin wrapper);
	 ...
	 * @param <DTO> 分页返回对象
	 */
	<DTO, P extends IPage<?>> IPage<DTO> selectJoinPage(P page, @Param(Constant.CLAZZ) Class<DTO> clazz,
			@Param(Constant.RESULT_MAP) String resultMap, @Param(Constants.WRAPPER) NeikoBaseJoin wrapper);
