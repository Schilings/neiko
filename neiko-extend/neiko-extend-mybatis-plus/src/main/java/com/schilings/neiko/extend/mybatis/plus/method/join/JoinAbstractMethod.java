...
public abstract class JoinAbstractMethod extends AbstractMethod implements JoinBaseMethod {
	...
	protected String sqlSelectColumns(TableInfo table, boolean queryWrapper) {
		...
		/**
		 * <script> <choose> <when test="ew != null and ew.sqlSelect != null>
		 * ${ew.sqlSelect} </when> <otherwize>t.field1,t.field2,....</otherwize> </choose>
		 * </script>
		 */
		// return SqlScriptUtils.convertChoose(String.format("%s != null and %s != null",
		// WRAPPER, Q_WRAPPER_SQL_SELECT),
	...
	protected String sqlCount() {
		 ...
		 * ${ew.sqlSelect} </when> <otherwise>*</otherwise> </choose> </script>
		 */
		// 相对于MP，多了ew.sqlSelect != ''
		return SqlScriptUtils.convertChoose(String.format("%s != null and %s != null and %s != ''", WRAPPER,
				Q_WRAPPER_SQL_SELECT, Q_WRAPPER_SQL_SELECT), SqlScriptUtils.unSafeParam(Q_WRAPPER_SQL_SELECT),
				ASTERISK);
	}
	...
	protected String sqlAlias() {
		/**
		 * <script> <if test="ew.alias != null and ew.alias != ''">${ew.alias}</if>
		 * </script>
		 */
		return SqlScriptUtils.convertIf("${ew.alias}", String.format("%s != null and %s != ''", "ew.alias", "ew.alias"),
				false);
	}
	...
	protected String sqlFrom() {
		/**
		 * <script> <if test="ew.from != null and ew.from != ''">${ew.from}</if> </script>
		 */
		return SqlScriptUtils.convertIf("${ew.from}", String.format("%s != null and %s != ''", "ew.from", "ew.from"),
				false);
	}
	...
	protected String sqlDistinct() {
		/**
		 * <script> <if test="ew.selectDistinct">DISTINCT</if> </script>
		 */
