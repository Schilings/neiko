...
public class NeikoQueryWrapper<T> extends AbstractWrapper<T, String, NeikoQueryWrapper<T>>
    ...
    /**
     * 连表关键字段，如LEFT JOIN t_user t1 ON t1.id=t.user_id LEFT JOIN t_role t2 ON t2.id=t.role_id
     */
     ...
     * 此方法只能用于主表
     * 不含主键
     * <p>参考 from {@link QueryWrapper#select(java.lang.Class, java.util.function.Predicate)}</p>
    ...
    public NeikoQueryWrapper<T> select(Class<T> entityClass, Predicate<TableFieldInfo> predicate) {
        ...
        selectColumns.addAll(info.getFieldList().stream()
                .filter(predicate)
                .map(c -> alias + StringPool.DOT + c.getColumn())
    ...
    public final NeikoQueryWrapper<T> selectAll(Class<?> clazz, String as) {
        ...
        selectColumns.addAll(info.getFieldList().stream()
                .map(i -> as + StringPool.DOT + i.getColumn())
                .collect(Collectors.toList()));
        return typedThis;
    }
    ...
    /**
     * 设置主表别名
     * 如果要用，请最先调用，因为selectAll(Class)会用到主表别名，默认为t，后面更改了的话，会造成找不到别名为t的表
     * <pre>
    ...
    /**
     * <p>{@link  com.schilings.neiko.extend.mabatis.plus.wrapper.join.NeikoJoin}</p>
     * <p>用于生成join sql</p>
     * <p>例如主表左联表1查询 leftJoin("t_user t1 on t1.id = t.user_id") --> left join t_user t1 on t1.id = t.user_id</p>
     * @param keyWord
     * @param condition
     * @param joinSql
     * @return
    ...
    protected NeikoQueryWrapper<T> instance() {
        return new NeikoQueryWrapper<>(getEntity(), getEntityClass(), paramNameSeq, paramNameValuePairs, new MergeSegments(),
