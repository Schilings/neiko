...
public abstract class BaseInsertBatch extends AbstractMethod {
	...
	/**
	 * <p>
	 * {@link Insert#injectMappedStatement(Class, Class, TableInfo)}
	 * </p>
	 * <p>
	 * {@link InsertBatchSomeColumn#injectMappedStatement(Class, Class, TableInfo))}
	 * </p>
	 * @param mapperClass
	 * @param modelClass
	 * @param tableInfo
	...
	protected String analyzedSql(TableInfo tableInfo) {
		...
		/**
		 * <script> INSERT INTO table field1,field2 VALUES
		 *
		 * <foreach collection="collection" item="item" index="index" open="(" sparator=
		 * "),(" close=")>"> #{item.field1},#{item.field2} </foreach> </script>
		 */
		return String.format(sql(), tableInfo.getTableName(), prepareFieldSql(tableInfo),
