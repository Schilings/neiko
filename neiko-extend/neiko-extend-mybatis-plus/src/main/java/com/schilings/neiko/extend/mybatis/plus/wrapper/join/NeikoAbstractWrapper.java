 ...
 *
 * <p>
 * 查询条件封装 copy from{@link AbstractWrapper}
 * </p>
 * <p>
 * 不过增加OnCompare的实现，join on条件的内容
 * </p>
 *
 * @author Schilings
 */

// T:实体类 R:列 Children:自身类型引用，链式调用
@SuppressWarnings("ALL")
public abstract class NeikoAbstractWrapper<T, Children extends NeikoAbstractWrapper<T, Children>> extends Wrapper<T>
	...
	@Override
	public <X, V> Children allEq(boolean condition, BiPredicate<SFunction<X, ?>, V> filter,
			Map<SFunction<X, ?>, V> params, boolean null2IsNull) {
		if (condition && CollectionUtils.isNotEmpty(params)) {
			params.forEach((k, v) -> {
				if (filter.test(k, v)) {
					if (StringUtils.checkValNotNull(v)) {
	...
	public Children apply(boolean condition, String applySql, Object... values) {
		return maybeDo(condition,
				() -> appendSqlSegments(APPLY, () -> formatSqlMaybeWithParam(applySql, null, values)));
	}
	...
	public <X> Children inSql(boolean condition, SFunction<X, ?> column, String inValue) {
		return maybeDo(condition,
				() -> appendSqlSegments(columnToSqlSegment(column), IN, () -> String.format("(%s)", inValue)));
	}

	@Override
	public <X> Children notInSql(boolean condition, SFunction<X, ?> column, String inValue) {
		return maybeDo(condition,
				() -> appendSqlSegments(columnToSqlSegment(column), NOT_IN, () -> String.format("(%s)", inValue)));
	}
	...
	public <R> Children orderByAsc(boolean condition, List<SFunction<R, ?>> columns) {
		return maybeDo(condition, () -> {
			...
			if (CollectionUtils.isNotEmpty(columns)) {
				columns.forEach(c -> appendSqlSegments(ORDER_BY, columnToSqlSegment(columnSqlInjectFilter(c)), mode));
			}
		});
	}
		...
		return maybeDo(condition, () -> {
			...
			if (ArrayUtils.isNotEmpty(columns)) {
				Arrays.stream(columns)
						.forEach(c -> appendSqlSegments(ORDER_BY, columnToSqlSegment(columnSqlInjectFilter(c)), mode));
			}
		});
	}
	...
	public Children having(boolean condition, String sqlHaving, Object... params) {
		return maybeDo(condition,
				() -> appendSqlSegments(HAVING, () -> formatSqlMaybeWithParam(sqlHaving, null, params)));
	}
	 ...
	 * </p>
	 */
	protected <X> Children likeValue(boolean condition, SqlKeyword keyword, SFunction<X, ?> column, Object val,
			SqlLike sqlLike) {
	...
	protected <X> Children addCondition(boolean condition, SFunction<X, ?> column, SqlKeyword sqlKeyword, Object val) {
		return maybeDo(condition,
				() -> appendSqlSegments(columnToSqlSegment(column), sqlKeyword, () -> formatParam(null, val)));
	}

	protected <X, S> Children addCondition(boolean condition, SFunction<X, ?> column, SqlKeyword sqlKeyword,
			SFunction<S, ?> val) {
		return maybeDo(condition,
				() -> appendSqlSegments(columnToSqlSegment(column), sqlKeyword, columnToSqlSegment(val)));
	}
	 ...
	 * @param mapping 例如:
	 * "javaType=int,jdbcType=NUMERIC,typeHandler=xxx.xxx.MyTypeHandler" 这种
	 * @param params 参数
	 * @return sql片段
	 */
	...
	@Override
	public <R, S, U> Children between(boolean condition, SFunction<R, ?> column, SFunction<S, ?> val1,
			SFunction<U, ?> val2) {
		return maybeDo(condition, () -> appendSqlSegments(columnToSqlSegment(column), BETWEEN, columnToSqlSegment(val1),
				AND, columnToSqlSegment(val2)));
	}
	...
	public <R, S> Children between(boolean condition, SFunction<R, ?> column, SFunction<S, ?> val1, Object val2) {
		return maybeDo(condition, () -> appendSqlSegments(columnToSqlSegment(column), BETWEEN, columnToSqlSegment(val1),
				AND, () -> formatParam(null, val2)));
	}

	@Override
	public <R, S, U> Children notBetween(boolean condition, SFunction<R, ?> column, SFunction<S, ?> val1,
			SFunction<U, ?> val2) {
		return maybeDo(condition, () -> appendSqlSegments(columnToSqlSegment(column), NOT_BETWEEN,
