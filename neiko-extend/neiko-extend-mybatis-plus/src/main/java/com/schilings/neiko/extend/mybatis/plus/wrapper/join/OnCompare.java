...
/**
 * 将原来的泛型R改成SFunction<R, ?>, Object改为SFunction<S, ?> 以及移除不会在ON语句中出现的条件 比如like相关 保留原来的like
 * 只是不太可能会出现 on a.id like b.id 所以不会支持这种写法
 * <p>
...
public interface OnCompare<Children> extends Serializable {
	 ...
	 * @return children
	 */
	<R, S, T> Children notBetween(boolean condition, SFunction<R, ?> column, SFunction<S, ?> val1,
			SFunction<T, ?> val2);
