...
/**
 * 参考 {@link com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper} Lambda
 * 语法使用 Wrapper
 * <p>
 */
public class NeikoLambdaQueryWrapper<T> extends NeikoAbstractLambdaWrapper<T, NeikoLambdaQueryWrapper<T>>
	...
	public final <S> NeikoLambdaQueryWrapper<T> select(SFunction<S, ?>... columns) {
		if (ArrayUtils.isNotEmpty(columns)) {
			for (SFunction<S, ?> s : columns) {
				selectColumns.add(
						SelectColumn.of(com.schilings.neiko.extend.mybatis.plus.utils.LambdaUtils.getEntityClass(s),
								getCache(s).getColumn()));
			}
		}
		return typedThis;
	}
	...
	public <S> NeikoLambdaQueryWrapper<T> selectAs(SFunction<S, ?> column, String alias) {
		selectColumns
				.add(SelectColumn.of(com.schilings.neiko.extend.mybatis.plus.utils.LambdaUtils.getEntityClass(column),
						getCache(column).getColumn(), alias));
		return typedThis;
	}

	public <S> NeikoLambdaQueryWrapper<T> selectFunc(boolean condition, BaseFuncEnum funcEnum, SFunction<S, ?> column,
			String alias) {
		if (condition) {
			selectColumns.add(
					SelectColumn.of(com.schilings.neiko.extend.mybatis.plus.utils.LambdaUtils.getEntityClass(column),
							getCache(column).getColumn(), alias, funcEnum));
		}
		return typedThis;
	}

	@Override
	public NeikoLambdaQueryWrapper<T> selectFunc(boolean condition, BaseFuncEnum funcEnum, Object column,
			String alias) {
		if (condition) {
	...
	public final NeikoLambdaQueryWrapper<T> selectAll(Class<?> clazz) {
		...
		info.getFieldList().forEach(c -> selectColumns.add(SelectColumn.of(clazz, c.getColumn())));
		return typedThis;
	}
	...
	public String getSqlSelect() {
		if (StringUtils.isBlank(sqlSelect.getStringValue())) {
			...
			String s = selectColumns.stream().map(i -> {
				String str = Constant.TABLE_ALIAS + getDefault(subTable.get(i.getClazz())) + StringPool.DOT
						+ i.getColumnName();
	...
	public String getFrom() {
		if (StringUtils.isBlank(from.getStringValue())) {
			...
			for (NeikoLambdaQueryWrapper<?> wrapper : onWrappers) {
				...
				value.append(wrapper.getKeyWord()).append(tableName).append(Constant.SPACE_TABLE_ALIAS)
						.append(subTable.get(wrapper.getJoinClass())).append(Constant.ON)
