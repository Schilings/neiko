    ...
    */
    ...
    public class SaTokenConfigure implements WebMvcConfigurer {
        ...
        public void addInterceptors(InterceptorRegistry registry) {
            ...
            registry.addInterceptor(new ExtendAnnotationInterceptor())
                    .addPathPatterns("/**")
...
//                        ? AnnotatedElementUtils.getAllMergedAnnotations(target, CACHE_OPERATION_ANNOTATIONS)
//                        : AnnotatedElementUtils.findAllMergedAnnotations(target, CACHE_OPERATION_ANNOTATIONS));
//                // 如果不存在注解声明
//                if (anns.isEmpty()) {
//                    return;
//                }
//
//                // 校验 @SaCheckLogin 注解
//                anns.stream().filter(ann -> resourceServerProperties.isCheckLogin() && ann instanceof SaCheckLogin).forEach(
...
//                        });
//                // 校验 @SaCheckRole 注解
//                anns.stream().filter(ann -> resourceServerProperties.isCheckRole() && ann instanceof SaCheckLogin).forEach(
//                        ann -> {
//                            SaCheckRole checkAnn = (SaCheckRole) ann;
//                            SaManager.getStpLogic(checkAnn.type()).checkByAnnotation(checkAnn);
//                        });
//                // 校验 @SaCheckPermission 注解
//                anns.stream().filter(ann -> resourceServerProperties.isCheckPermission() && ann instanceof SaCheckPermission).forEach(
//                        ann -> {
//                            SaCheckPermission checkAnn = (SaCheckPermission) ann;
//                            SaManager.getStpLogic(checkAnn.type()).checkByAnnotation(checkAnn);
//                        });
//                // 校验 @SaCheckSafe 注解
//                anns.stream().filter(ann -> resourceServerProperties.isCheckSafe() && ann instanceof SaCheckSafe).forEach(
//                        ann -> {
//                            SaCheckSafe checkAnn = (SaCheckSafe) ann;
//                            SaManager.getStpLogic(checkAnn.type()).checkByAnnotation(checkAnn);
//                        });
//                // 校验 @SaCheckBasic 注解
//                anns.stream().filter(ann -> resourceServerProperties.isCheckBasic() && ann instanceof SaCheckBasic).forEach(
//                        ann -> {
//                            SaCheckBasic checkAnn = (SaCheckBasic) ann;
//                            SaBasicUtil.check(checkAnn.realm(), checkAnn.account());
//                        });
//                // 校验 @OAuth2CheckScope 注解
//                anns.stream().filter(ann -> resourceServerProperties.isCheckBasic() && ann instanceof OAuth2CheckScope).forEach(
//                        ann -> {
//                            OAuth2CheckScope checkAnn = (OAuth2CheckScope) ann;
//                            ((StpOAuth2Logic) SaManager.getStpLogic(checkAnn.type())).checkByAnnotation(checkAnn);
//                        });
//            };
        }
