...
/**
 * <p>
 * 先copy from SaOAuth2Handle
 * </p>
 * <p>
 * 拓展Sa-Token原有Oauth2认证逻辑
 * </p>
 *
 * @author Schilings
 */
public class ExtendOAuth2Handler {
	...
	public static Object serverRequest() {
		...
		if (req.isPath(SaOAuth2Consts.Api.authorize)
				&& req.isParam(SaOAuth2Consts.Param.response_type, SaOAuth2Consts.ResponseType.code)) {
		...
		if (req.isPath(SaOAuth2Consts.Api.token)
				&& req.isParam(SaOAuth2Consts.Param.grant_type, SaOAuth2Consts.GrantType.authorization_code)) {
			return token(req, res, cfg);
		}

		// Refresh-Token 刷新 Access-Token
		if (req.isPath(SaOAuth2Consts.Api.refresh)
				&& req.isParam(SaOAuth2Consts.Param.grant_type, SaOAuth2Consts.GrantType.refresh_token)) {
	...
	/**
	 * 模式三：密码式(重写) 添加校验client_secret;token增强
	 * @param req 请求对象
	 * @param res 响应对象
	 * @param cfg 配置对象
	 * @return 处理结果
