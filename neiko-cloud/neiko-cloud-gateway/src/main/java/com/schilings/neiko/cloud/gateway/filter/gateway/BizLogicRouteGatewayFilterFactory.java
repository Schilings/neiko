 ...
 *
 * 在apply方法中，重新创建ServerHttpRequest和Route对象，它们的参数可以按照业务需求随意设置， 然后再将这两个对象设置给SpringCloud
 * gateway的处理链中，接下来，处理链上的其他过滤拿到的就是新的ServerHttpRequest和Route对象了
 *
 */

@Slf4j
public class BizLogicRouteGatewayFilterFactory
		extends AbstractGatewayFilterFactory<BizLogicRouteGatewayFilterFactory.Config> {
	...
	public GatewayFilter apply(Config config) {
		...
		return (exchange, chain) -> {
			...
			if (isTestUser) {
				log.info("这是测试用户的请求");
				// 从配置文件中得到测试环境的uri
				uri = UriComponentsBuilder.fromHttpUrl(config.getDevEnvUri() + rawPath).queryParams(queryParams).build()
						.toUri();
			}
			else {
				log.info("这是普通用户的请求");
				// 从配置文件中得到正式环境的uri
				uri = UriComponentsBuilder.fromHttpUrl(config.getProdEnvUri() + rawPath).queryParams(queryParams)
						.build().toUri();
			}

			// 生成新的Request对象，该对象放弃了常规路由配置中的spring.cloud.gateway.routes.uri字段
			ServerHttpRequest serverHttpRequest = request.mutate().uri(uri).method(httpMethod)
					.headers(httpHeaders -> httpHeaders = httpHeaders).build();
