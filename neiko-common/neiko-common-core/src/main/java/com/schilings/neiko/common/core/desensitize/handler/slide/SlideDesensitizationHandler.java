...
public interface SlideDesensitizationHandler extends DesensitizationHandler {
	...
	default String handle(Annotation annotation, String origin) {
		...
		if (type.equals(SlideDesensitizationTypeEnum.CUSTOM)) {
			...
			return handle(origin, slideAnnotation.leftPlainTextLen(), slideAnnotation.rightPlainTextLen(),
					slideAnnotation.maskString());
		}
		return handle(origin, type);
	}

	default void checkValue(JsonSlideDesensitize annotation) {
		Assert.state(annotation.leftPlainTextLen() >= 0,
				"The leftPlainTextLen of JsonSlideDesensitize can not be less than zeros.");
		Assert.state(annotation.rightPlainTextLen() >= 0,
				"The rightPlainTextLen of JsonSlideDesensitize can not be less than zeros.");
