 ...
 */
@Slf4j
public abstract class AbstractEventHandlerMethodMapping<T extends EventMappingInfo> extends AbstractEventHandlerMapping
	...
	protected void detectHandlerMethods(Object handler) {
		...
		if (handlerType != null) {
			...
			// 遍历这个类中的方法，进行映射绑定
			Set<Method> methods = MethodIntrospector.selectMethods(userType, (ReflectionUtils.MethodFilter) method -> {
			...
			methods.forEach(m -> {
				try {
					...
					throw new IllegalStateException(
							"Invalid mapping on handler class [" + userType.getName() + "]: " + m, ex);
				}
			});
	...
	public Map<T, HandlerMethod> getHandlerMethods() {
		...
		try {
			return Collections.unmodifiableMap(this.mappingRegistry.getRegistrations().entrySet().stream()
	...
	private String formatMappings(Class<?> userType, Map<Method, T> methods) {
		...
		String formattedType = (StringUtils.hasText(packageName)
				? Arrays.stream(packageName.split("\\.")).map(packageSegment -> packageSegment.substring(0, 1)).collect(
	...
	public class EventMappingRegistry {
		...
		public void register(T mapping, Object handler, Method method) {
			...
			try {
				...
				EventMappingRegistration<T> registration = new EventMappingRegistration<>(mapping, handlerMethod,
						mapping.getUniqueName(), methodAdater);
	...
	public static class EventMappingRegistration<T> {
		...
		EventMappingRegistration(T mappingInfo, HandlerMethod handlerMethod, String mappingName,
