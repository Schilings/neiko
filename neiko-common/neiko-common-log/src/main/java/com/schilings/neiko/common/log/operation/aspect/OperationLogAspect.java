...
public class OperationLogAspect<T> {
    ...
    private void handleLog(ProceedingJoinPoint joinPoint, long startTime, T logDto, 
                           Throwable throwable, boolean isSaveResult, Object result) {
        try {
            // 结束时间
            long executionTime = System.nanoTime() - startTime;
            // 记录执行信息
            handler.recordExecutionInfo(logDto, joinPoint, TimeUnit.NANOSECONDS.toSeconds(executionTime), 
                    throwable, isSaveResult, result);
            // 处理操作日志
