...
public @interface NeikoCacheEvict {
	...
	/**
	 * 是否删除缓存内的所有数据。 默认情况下，仅删除关联键下的值。 请注意，不允许将此参数设置为true并指{@link #key}
	 */
	boolean allEntries() default false;

	/**
	 * 是否应该在调用方法之前进行删除缓存。 将此属性设置为true ，无论方法结果如何（即，它是否抛出异常）都会导致缓存删除的发生。 默认为false
	 * ，这意味着缓存驱逐操作将在成功调用建议的方法后发生（即仅当调用未引发异常时）。
	 * @return
	 */
