...
public class NeikoCacheAnnotationParser implements CacheAnnotationParser, Serializable {
    ...
    /**
     * CacheOperations 的缓存，由特定目标类上的方法键入。
     * 缓存将在序列化后重新创建——前提是 Serializable。
     */
    ...
    /**
     * 确定给定类是否解析类方法中的缓存操作的候选对象。
     * 如果此方法返回false ，则不会遍历给定类上的方法以进行getCacheOperations内省。
     ...
     */
    @Nullable
    private Collection<CacheOperation> parseCacheAnnotations(DefaultCacheConfig cachingConfig,
    ...
    private CacheableOperation parseCacheableAnnotation(
            AnnotatedElement ae, DefaultCacheConfig defaultConfig, NeikoCacheable cacheable) {
    ...
    private static class DefaultCacheConfig {
        ...
        public void applyDefault(CacheOperation.Builder builder) {
            if (!this.initialized) {
                NeikoCacheConfig annotation = AnnotatedElementUtils.findMergedAnnotation(this.target, NeikoCacheConfig.class);
                if (annotation != null) {
