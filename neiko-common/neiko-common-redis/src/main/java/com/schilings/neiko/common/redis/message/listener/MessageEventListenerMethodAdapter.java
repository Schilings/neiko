...
public class MessageEventListenerMethodAdapter implements MessageEventListener {
	...
	private Topic resolveDeclaredTopic(RedisListener ann) {
		if (ann != null) {
			...
			Assert.hasText(topic, "The topic of the message listener can not be empty.");
	...
	private static List<ResolvableType> resolveParameterTypes(Method method) {
		...
		if (count == 1) {
			...
			types = Arrays.asList(ResolvableType.forMethodParameter(method, 0),
					ResolvableType.forMethodParameter(method, 1));
		}
		return types;
	}
	...
	protected static void validMethod(Method method, List<ResolvableType> types) {
		...
		if (!Void.TYPE.isAssignableFrom(returnType)) {
			throw new IllegalStateException("Return type is must be void for event listener method: " + method);
		}
		// 第一个参数类型
