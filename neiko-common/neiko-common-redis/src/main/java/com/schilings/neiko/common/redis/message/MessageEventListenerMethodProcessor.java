...
*/
@Slf4j
public class MessageEventListenerMethodProcessor implements
    ...
    /**
     * <p>由于{@link MessageEventListenerMethodProcessor}实现{@link BeanFactoryPostProcessor}</p>
     * <p>所以{@link MessageEventListenerMethodProcessor}会被提前进行创建bean，然而依赖的{@link RedisMessageListenerContainer}bean</p>
     * <p>还没注入到容器中（配置类已加载解析但还未创建对应的Bean方法）,就会触发{@link MessageEventListenerAutoConfiguration}生效
     * 去创建{@link RedisMessageListenerContainer}</p>
     * <p>但是由于{@link RedisMessageListenerContainer}依赖{@link RedisConnectionFactory},再去触发{@link RedisAutoConfiguration}</p>
     * <p>造成{@link RedisAutoConfiguration}在{@link MessageEventListenerAutoConfiguration}之后加载，Redis的自动配置内容未生效，因此报错</p>
     *  
     * <p>因此目前方案是在{@link MessageEventListenerMethodProcessor#afterSingletonsInstantiated()}内手动注入</p>
     * <p>还有很多其他方案，总之拿到BeanFactory手动注入</p>
     *  
     */
   
   
    @Setter//RedisMessageListenerContainer container = beanFactory.getBean(RedisMessageListenerContainer.class);
    private RedisMessageListenerContainer listenerContainer;
    @Setter// RedisTemplate template = beanFactory.getBean("redisTemplate",RedisTemplate.class);
    ...
    public void postProcessBeanFactory(ConfigurableListableBeanFactory beanFactory) throws BeansException {
        ...
        Map<String, MessageEventListenerFactory> beans =
                beanFactory.getBeansOfType(MessageEventListenerFactory.class, false, false);
    ...
    public void afterSingletonsInstantiated() {
        ...
        try {
            ...
            if (log.isDebugEnabled()) {
                log.error("Could not find RedisMessageListenerContainer bean for MessageEventListenerMethodProcessor with name ", e);
            }
            throw e;
        }
            ...
            if (!ScopedProxyUtils.isScopedTarget(beanName)) {
                ...
                if (type != null) {
                    if (ScopedObject.class.isAssignableFrom(type)) {
                        try {
                            Class<?> targetClass =
                                    AutoProxyUtils.determineTargetClass(beanFactory, ScopedProxyUtils.getTargetBeanName(beanName));
                            if (targetClass != null) {
                                type = targetClass;
                            }
                        } catch (Throwable ex) {
                    ...
                    try {
                        ...
                        throw new BeanInitializationException("Failed to process @RedisListener " +
    ...
    private void processBean(final String beanName,final Class<?> targetType) {
        ...
        if (!this.nonAnnotatedClasses.contains(targetType) && 
            ...
            try {
                annotatedMethods = MethodIntrospector.selectMethods(targetType,
                        (MethodIntrospector.MetadataLookup<RedisListener>) method ->
                                AnnotatedElementUtils.findMergedAnnotation(method, RedisListener.class));
            } catch (Throwable ex) {
            ...
            if (CollectionUtils.isEmpty(annotatedMethods)) {
                ...
                for (Method method : annotatedMethods.keySet()) {
                    for (MessageEventListenerFactory factory : factories) {
                        ...
                        if (factory.supportsMethod(method)) {
                            ...
                            MessageEventListener messageListener = factory.createMessageEventListener(beanName, targetType, methodToUse);
                            if (messageListener instanceof MessageEventListenerMethodAdapter) {
                                //初始化实例
                                ((MessageEventListenerMethodAdapter) messageListener).init(applicationContext, evaluator,redisSerializer);
                            }
                            //添加到容器
                ...
                if (log.isDebugEnabled()) {
                    log.debug(annotatedMethods.size() + " @RedisListener methods processed on bean '" +
