 ...
 * {@link #isAccessAllowed(ServletRequest, ServletResponse, Object) isAccessAllowed} -> <br>
...
public class JWTAuthenticationFilter extends BasicHttpAuthenticationFilter {
    ...
    /**
     * 通过 {@link #isLoginAttempt(ServletRequest, ServletResponse) isLoginAttempt} 判断是否有携带token，如果携带则验证token是否合法，如果合法则调用认证，如果没有携带token或者token不合法、认证失败等都中断请求，执行{@link #onAccessDenied(ServletRequest, ServletResponse) onAccessDenied}
     */
    @Override
    protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue) {
        ...
        log.error("Attempting to access '{}' which requires authentication. But no Java Web Token was found in the request whatever header, param or cookie.", ((HttpServletRequest) request).getRequestURI());
        return false;
    }
