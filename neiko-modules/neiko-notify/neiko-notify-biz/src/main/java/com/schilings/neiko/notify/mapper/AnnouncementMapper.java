...
public interface AnnouncementMapper extends ExtendMapper<Announcement> {
	...
	default PageResult<AnnouncementPageVO> queryPage(PageParam pageParam, AnnouncementQO qo) {
		...
		NeikoLambdaQueryWrapper<Object> queryWrapper = WrappersX.lambdaQueryJoin().selectAll(Announcement.class)
		...
		IPage<AnnouncementPageVO> voPage = this.selectJoinPage(page, AnnouncementPageVO.class, AUTO_RESULT_MAP,
				queryWrapper);
	...
	default List<Announcement> listUserAnnouncements(Long userId, boolean pulled) {
		 ...
		 * SELECT <include refid="Base_Alias_Column_List"/> FROM notify_announcement a
		 * LEFT JOIN notify_user_announcement ua ON ua.user_id = #{userId} AND
		 * ua.announcement_id = a.id WHERE a.STATUS = 1 AND ( a.immortal = 1 OR a.deadline
		 * > now() ) <choose> <when test="pulled"> AND ua.id IS NOT NULL </when>
		 * <otherwise> AND ua.id IS NULL </otherwise> </choose> </select>
		 */
		NeikoLambdaQueryWrapper<Object> queryWrapper = WrappersX.lambdaQueryJoin().selectAll(Announcement.class)
				.leftJoin(UserAnnouncement.class, UserAnnouncement::getAnnouncementId, Announcement::getId)
				.eq(Announcement::getDeleted, GlobalConstants.NOT_DELETED_FLAG).eq(UserAnnouncement::getUserId, userId)
				.eq(Announcement::getStatus, AnnouncementStatusEnum.ENABLED.getValue()).and(wrapper -> {
					wrapper.eq(Announcement::getImmortal, BooleanEnum.TRUE.getValue()).or()
							// mysql内置函数now()会被渲染为"now()",多了双引号，查询失败，待修改
							.gt(Announcement::getDeadline, "2021-09-23 17:09:18");
				}).isNotNull(pulled, UserAnnouncement::getId).isNull(!pulled, UserAnnouncement::getId);
