...
public class AuthorizationServiceImpl extends ExtendServiceImpl<AuthorizationMapper, Authorization>
	...
	public Authorization getByState(String state) {
		List<Authorization> authorizations = baseMapper
				.selectList(WrappersX.<Authorization>lambdaQueryX().eq(Authorization::getState, state));
	...
	public Authorization getByAccessToken(String accessTokenValue) {
		List<Authorization> authorizations = baseMapper.selectList(
				WrappersX.<Authorization>lambdaQueryX().eq(Authorization::getAccessTokenValue, accessTokenValue));
		return !authorizations.isEmpty() ? authorizations.get(0) : null;
	}
	...
	public Authorization getByRefreshToken(String refreshTokenValue) {
		List<Authorization> authorizations = baseMapper.selectList(
				WrappersX.<Authorization>lambdaQueryX().eq(Authorization::getRefreshTokenValue, refreshTokenValue));
		return !authorizations.isEmpty() ? authorizations.get(0) : null;
	}

	@Override
	public Authorization getIfUnkonwTokenType(String state, String authorizationCodeValue, String accessTokenValue,
			String refreshTokenValue) {
		List<Authorization> authorizations = baseMapper
				.selectList(WrappersX.<Authorization>lambdaQueryX().eq(Authorization::getState, state).or()
