...
public class OAuth2RegisteredClientUtils {
    ...
    public static ClientSettings toClientSettings(OAuth2ClientSettingsVO vo) {
        ...
        ClientSettings.Builder builder = ClientSettings.builder()
                .requireProofKey(vo.getRequireProofKey() == BooleanEnum.TRUE.getValue())
                .requireAuthorizationConsent(vo.getRequireAuthorizationConsent() == BooleanEnum.TRUE.getValue());
        SignatureAlgorithm signatureAlgorithm = SignatureAlgorithm.from(vo.getSigningAlgorithm());
        JwsAlgorithm jwsAlgorithm = signatureAlgorithm == null ? MacAlgorithm.from(vo.getSigningAlgorithm()) : signatureAlgorithm;
    ...
    public static OAuth2ClientSettingsDTO fromClientSettings(ClientSettings clientSettings) {
        ...
        dto.setRequireProofKey(Optional.of(clientSettings.isRequireProofKey()).filter(bool -> bool).map(bool -> BooleanEnum.TRUE.getValue()).orElse(BooleanEnum.FALSE.getValue()));
        dto.setRequireAuthorizationConsent(Optional.of(clientSettings.isRequireAuthorizationConsent()).filter(bool -> bool).map(bool -> BooleanEnum.TRUE.getValue()).orElse(BooleanEnum.FALSE.getValue()));
    ...
    public static TokenSettings toTokenSettings(OAuth2TokenSettingsVO vo) {
        ...
        return TokenSettings.builder()
                ...
                .reuseRefreshTokens(vo.getReuseRefreshTokens() == BooleanEnum.TRUE.getValue())
    ...
    public static OAuth2TokenSettingsDTO fromTokenSettings(TokenSettings tokenSettings) {
        ...
        dto.setReuseRefreshTokens(tokenSettings.isReuseRefreshTokens() ? BooleanEnum.TRUE.getValue() : BooleanEnum.FALSE.getValue());
        dto.setIdTokenSignatureAlgorithm(Optional
                .ofNullable(tokenSettings.getIdTokenSignatureAlgorithm())
                .map(SignatureAlgorithm::getName)
                .orElse(SignatureAlgorithm.RS256.getName()));
        return dto;
    }
