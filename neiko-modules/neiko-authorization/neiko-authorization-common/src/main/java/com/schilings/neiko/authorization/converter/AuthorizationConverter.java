...
public interface AuthorizationConverter {
	...
	@Mappings(value = {
			@Mapping(target = "authorizationCodeIssuedAt",
					expression = "java(stringToInstant(po.getAuthorizationCodeExpiresAt()))"),
			@Mapping(target = "authorizationCodeExpiresAt",
					expression = "java(stringToInstant(po.getAuthorizationCodeExpiresAt()))"),
			@Mapping(target = "accessTokenIssuedAt", expression = "java(stringToInstant(po.getAccessTokenIssuedAt()))"),
			@Mapping(target = "accessTokenExpiresAt",
					expression = "java(stringToInstant(po.getAccessTokenExpiresAt()))"),
			@Mapping(target = "refreshTokenIssuedAt",
					expression = "java(stringToInstant(po.getRefreshTokenIssuedAt()))"),
			@Mapping(target = "refreshTokenExpiresAt",
					expression = "java(stringToInstant(po.getRefreshTokenExpiresAt()))"),
			@Mapping(target = "oidcIdTokenIssuedAt", expression = "java(stringToInstant(po.getOidcIdTokenIssuedAt()))"),
			@Mapping(target = "oidcIdTokenExpiresAt",
					expression = "java(stringToInstant(po.getOidcIdTokenExpiresAt()))"),
			@Mapping(target = "accessTokenScopes",
					expression = "java(commaDelimitedListToSet(po.getAccessTokenScopes()))"),
