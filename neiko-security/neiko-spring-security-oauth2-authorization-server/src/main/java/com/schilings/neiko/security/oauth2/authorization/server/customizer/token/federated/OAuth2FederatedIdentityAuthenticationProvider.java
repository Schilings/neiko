...
/**
 * 
 * <p>{@link OAuth2AuthorizationCodeRequestAuthenticationProvider}</p>
 * <p>{@link OAuth2AuthorizationCodeAuthenticationProvider}</p>
 * 
 * @author Schilings
*/
public class OAuth2FederatedIdentityAuthenticationProvider implements OAuth2ExtensionGrantTypeAuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        OAuth2FederatedIdentityAuthenticationToken federatedIdentityAuthenticationToken = 
                (OAuth2FederatedIdentityAuthenticationToken) authentication;
        
        OAuth2ClientAuthenticationToken clientPrincipal =
                getAuthenticatedClientElseThrowInvalidClient(federatedIdentityAuthenticationToken);
        ...
        if (!registeredClient.getClientId().equals(authorization.getAttribute(OAuth2FederatedIdentityConstant.CLIENT_ID))) {
            throw new OAuth2AuthenticationException(OAuth2ErrorCodes.INVALID_CLIENT);
        }
        
        //校验授权方式GrantType
        if (!registeredClient.getAuthorizationGrantTypes().contains(OAuth2FederatedIdentityConstant.FEDERATED_IDENTITY)) {
        ...
        OAuth2Authorization.Token<OAuth2FederatedIdentityCode> federatedIdentityCodeToken =
                authorization.getToken(OAuth2FederatedIdentityCode.class);
        ...
        //使授权代码无效，因为它只能使用一次
        authorization = OAuth2AuthenticationProviderUtils.invalidate(authorization, federatedIdentityCodeToken.getToken());
        //认证结束，持久化
