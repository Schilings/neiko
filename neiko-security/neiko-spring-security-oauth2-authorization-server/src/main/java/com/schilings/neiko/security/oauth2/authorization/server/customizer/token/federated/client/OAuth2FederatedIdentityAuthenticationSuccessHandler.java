...
public class OAuth2FederatedIdentityAuthenticationSuccessHandler implements AuthenticationSuccessHandler {
    ...
    public OAuth2FederatedIdentityAuthenticationSuccessHandler(OAuth2AuthorizationService authorizationService, RegisteredClientRepository registeredClientRepository) {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException, ServletException {
        ...
        RegisteredClient registeredClient = this.registeredClientRepository.findByClientId(authorizationRequest.getAttribute(OAuth2FederatedIdentityConstant.CLIENT_ID));
        if (registeredClient == null) {
            OAuth2Error error = new OAuth2Error(OAuth2ErrorCodes.INVALID_REQUEST,
                    "The OAuth2 Federated Request Authorization Request failed to check the " + OAuth2FederatedIdentityConstant.CLIENT_ID, "");
            throw new OAuth2AuthenticationException(error);
        }

        //redirect_uri,检验RegisteredClient是否有该redirect_uri
        if (!registeredClient.getRedirectUris().contains((String) authorizationRequest.getAttribute(OAuth2FederatedIdentityConstant.REDIRECT_URI))) {
        ...
        OAuth2Authorization authorization = authorizationBuilder(registeredClient, authenticationToken,authorizationRequest)
    ...
    private static OAuth2Authorization.Builder authorizationBuilder(RegisteredClient devClient, OAuth2AuthenticationToken authenticationToken, OAuth2AuthorizationRequest authorizationRequest) {         
        ...
        return OAuth2Authorization.withRegisteredClient(devClient)
                ...
                //放入registrationId
                .attribute(OAuth2FederatedIdentityConstant.REGISTRATION_ID, authenticationToken.getAuthorizedClientRegistrationId())
                //放入redirect_uri
                .attribute(OAuth2FederatedIdentityConstant.REDIRECT_URI, authorizationRequest.getAttribute(OAuth2FederatedIdentityConstant.REDIRECT_URI));
    }


    private static OAuth2TokenContext createTokenContext(RegisteredClient devClient, OAuth2AuthenticationToken authenticationToken) {
        // @formatter:off
    ...
    private void sendFederatedIdentityResponse(HttpServletRequest request, HttpServletResponse response,
                                           OAuth2FederatedIdentityCode federatedIdentityCode,OAuth2AuthorizationRequest authorizationRequest) throws IOException {
        //携带参数转发到redirect_url
        ...
        if (Objects.nonNull(federatedIdentityCode.getExpiresAt())) {
            ...
            queryParams.put(OAuth2FederatedIdentityConstant.EXPIRES_IN, String.valueOf(getExpiresIn(federatedIdentityCode)));
