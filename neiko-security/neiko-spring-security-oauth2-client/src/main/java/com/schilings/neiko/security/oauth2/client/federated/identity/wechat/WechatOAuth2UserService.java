...
public class WechatOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
	...
	public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
		...
		// 微信MP网页端授权获取UserInfo，请求需要携带OpenId字段
		...
		// http请求
		ResponseEntity<WechatOAuth2User> response = getResponse(userRequest);
		WechatOAuth2User auth2User = response.getBody();
	...
	private ResponseEntity<WechatOAuth2User> getResponse(OAuth2UserRequest userRequest) {
		...
		try {
			...
			URI userInfoEndpoint = UriComponentsBuilder.fromUriString(userInfoUri).queryParams(queryParams).build()
					.toUri();
			...
			String errorMessage = "An error occurred while attempting to retrieve the UserInfo Resource from '"
					+ userInfoUri + "': response contains invalid content type '" + ex.getContentType() + "'. "
	...
	private static class WechatOAuth2UserHttpMessageConverter extends AbstractHttpMessageConverter<WechatOAuth2User> {
		...
		public WechatOAuth2UserHttpMessageConverter() {
			super(DEFAULT_CHARSET, MediaType.TEXT_PLAIN, MediaType.APPLICATION_JSON,
		...
		@Override
		protected WechatOAuth2User readInternal(Class<? extends WechatOAuth2User> clazz, HttpInputMessage inputMessage)
				throws HttpMessageNotReadableException {
			try {
				// gh-6463: Parse parameter values as Object in order to handle potential
				// JSON
				// Object and then convert values to String
				return (WechatOAuth2User) this.jsonMessageConverter.read(OAUTH2_USER_OBJECT.getType(), null,
						inputMessage);
