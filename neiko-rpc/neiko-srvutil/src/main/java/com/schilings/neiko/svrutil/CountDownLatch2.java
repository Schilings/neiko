...
public class CountDownLatch2 {
	 ...
	 * @param count the number of times {@link #countDown} must be invoked before threads
	 ...
	 * </ul>
	 * then {@link InterruptedException} is thrown and the current thread's interrupted
	 * status is cleared.
	...
	/**
	 * Causes the current thread to wait until the latch has counted down to zero, unless
	 * the thread is {@linkplain Thread#interrupt interrupted}, or the specified waiting
	 * time elapses.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * If the specified waiting time elapses then the value {@code false} is returned. If
	 * the time is less than or equal to zero, the method will not wait at all.
	 * @param timeout the maximum time to wait
	 * @param unit the time unit of the {@code timeout} argument
	 * @return {@code true} if the count reached zero and {@code false} if the waiting
	 ...
	 *
	 * <p>
	 * If the current count is greater than zero then it is decremented. If the new count
	 * is zero then all waiting threads are re-enabled for thread scheduling purposes.
	 *
	 * <p>
	 ...
	 * brackets, includes the String {@code "Count ="} followed by the current count.
