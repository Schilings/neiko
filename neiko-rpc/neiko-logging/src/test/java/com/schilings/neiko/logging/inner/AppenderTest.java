...
public class AppenderTest extends BasicLoggerTest {
	...
	public void testInnerFile() throws IOException {
		...
		Appender myappender = LoggingBuilder.newAppenderBuilder().withDailyFileRollingAppender(file, "'.'yyyy-MM-dd")
				.withName("myappender").withLayout(LoggingBuilder.newLayoutBuilder().withDefaultLayout().build())
				.build();
	...
	public void testFileAppender() throws IOException {
		LoggingBuilder.FileAppender fileAppender = new LoggingBuilder.FileAppender(new LoggingBuilder.SimpleLayout(),
				loggingDir + "/simple.log", true);
