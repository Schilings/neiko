...
public class NeikoDefaultSerializable {

	private static final Charset CHARSET_UTF8 = Charset.forName("UTF-8");

	public static byte[] neikoProtocolEncode(RemotingCommand cmd) {
		// String remark
	...
	public static RemotingCommand neikoProtocolDecode(final byte[] headerArray) throws RemotingCommandException {
		...
		if (remarkLength > 0) {
			if (remarkLength > headerArray.length) {
				throw new RemotingCommandException("RocketMQ protocol decoding failed, remark length: " + remarkLength
						+ ", but header length: " + headerArray.length);
			}
		...
		if (extFieldsLength > 0) {
			if (extFieldsLength > headerArray.length) {
				throw new RemotingCommandException("RocketMQ protocol decoding failed, extFields length: "
						+ extFieldsLength + ", but header length: " + headerArray.length);
			}
