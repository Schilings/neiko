...
import static com.schilings.neiko.remoting.netty.TlsSystemConfig.*;
		...
		if (forClient) {
			if (tlsTestModeEnable) {
				...
				return sslContextBuilder
						.keyManager(!isNullOrEmpty(tlsClientCertPath) ? new FileInputStream(tlsClientCertPath) : null,
			...
			if (tlsTestModeEnable) {
				...
				return SslContextBuilder
						...
						.sslProvider(provider).clientAuth(ClientAuth.OPTIONAL).build();
			}
			else {
